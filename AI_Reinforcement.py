...
class Q_Ai:
    ...
    def __init__(self, line_number, exploitation=0.95, learn_rate=0.01, discount=0.9):
        ...
        self.create_or_load_table(line_number)
        self.guide = self.create_vector_array(self.line_number)
        
    def create_or_load_table(self, possibility_number):
        ...
        try:
            self.filename = "Luck_game_agent["+str(possibility_number)+"].pkl"
    ...
    def update_Q_table(self, state, line, reward):
        ...
        if self.Q_table[line][column] > 3:
            self.Q_table[line][column] = 3
        if self.Q_table[line][column] < -3:
            self.Q_table[line][column] = -3
    ...
    def heatmap(self):
        ...
        arr = np.arange(0, self.line_number, int(self.line_number/10))
